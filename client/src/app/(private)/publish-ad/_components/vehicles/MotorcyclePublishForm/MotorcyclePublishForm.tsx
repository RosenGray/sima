...
} from "@/lib/vehicles/motorcycles/types/motorcycle.schema";
...
import { publishMotorcycleAd } from "@/lib/vehicles/motorcycles/actions/publishMotorcycleAd";
import { editMotorcycleAd } from "@/lib/vehicles/motorcycles/actions/editMotorcycleAd";
...
import {
  ...
  HeroCard,
  SectionCard,
} from "./MotorcyclePublishForm.styles";
import { mapMotorcycleManufacturersToSelectOptions } from "@/lib/vehicles/motorcycles/motorcycleManufacturers";
import { getMotorcycleModelsToSelectOptions } from "@/lib/vehicles/motorcycles/motorcycleModels";
...
// Map MotorcycleKind enum to select options
const kindOptions = Object.values(MotorcycleKind).map((value) => ({
  value,
  label:
    value === MotorcycleKind.MOTOCROSS
      ? "Мотокросс"
      : value === MotorcycleKind.DUAL_PURPOSE
      ? "Двойного назначения"
      : value === MotorcycleKind.SPORT_ROAD
      ? "Спорт / дорога"
      : "Другой",
  fieldKey: "kind",
}));

interface MotorcyclePublishFormProps {
  motorcycle?: SerializedMotorcycle;
  formMode: FormMode;
}

const MotorcyclePublishForm: FC<MotorcyclePublishFormProps> = ({
  motorcycle,
  formMode,
}) => {
  ...
  const updateMotorcycleWithImagesToDelete = editMotorcycleAd.bind(null, {
    motorcyclePublicId: motorcycle?.publicId as string,
    imagesToDelete,
    allImagesShouldBeDeleted,
  });

  const [formState, formAction, isPending] = useActionState(
    isCreateMode ? publishMotorcycleAd : updateMotorcycleWithImagesToDelete,
    undefined
  );
  ...
  const [form, fields] = useForm({
    defaultValue: {
      ...
      yearOfManufacture: motorcycle?.yearOfManufacture?.toString() || "",
      numberOfHand: motorcycle?.numberOfHand?.toString() || "",
      kind: motorcycle?.kind || "",
      mileage: motorcycle?.mileage?.toString() || "",
      price: motorcycle?.price?.toString() || "",
      description: motorcycle?.description || "",
      district: motorcycle?.district || Districts.Center,
      city: motorcycle?.city || "",
      contactName: motorcycle?.contactName || "",
      contactPrimaryPhone: motorcycle?.contactPrimaryPhone || "",
      contactSecondaryPhone: motorcycle?.contactSecondaryPhone || "",
      contactEmail: motorcycle?.contactEmail || user?.email || "",
      acceptTerms: motorcycle?.acceptTerms ? "on" : null,
      images: [],
    },
  ...
  const manufacturerOptions = useMemo(
    () => mapMotorcycleManufacturersToSelectOptions(),
    []
  );
  ...
  const modelOptions = useMemo(
    () =>
      manufacturer.value
        ? getMotorcycleModelsToSelectOptions(manufacturer.value as string)
        : [],
    [manufacturer.value]
  );
            ...
            <SectionCard variant="surface" size="4">
              <Flex direction="column" gap={{ initial: "4", md: "5" }}>
                <Box>
                  ...
                  <Text color="gray" size="2" mt="2">
                    Укажите цену и подробное описание мотоцикла.
                  </Text>
                </Box>
                <Grid
                  columns={{ initial: "1", md: "2" }}
            ...
            <SectionCard variant="surface" size="4">
              <Flex direction="column" gap={{ initial: "4", md: "5" }}>
                <Box>
                  ...
                  <Text color="gray" size="2" mt="2">
                    Укажите район и город, где находится мотоцикл.
                  </Text>
                </Box>
                <Grid
                  columns={{ initial: "1", md: "2" }}
            ...
            <SectionCard variant="surface" size="4">
              <Flex direction="column" gap={{ initial: "4", md: "5" }}>
                <Box>
                  ...
                  <Text color="gray" size="2" mt="2">
                    Добавьте изображения мотоцикла.
...
export default MotorcyclePublishForm;
