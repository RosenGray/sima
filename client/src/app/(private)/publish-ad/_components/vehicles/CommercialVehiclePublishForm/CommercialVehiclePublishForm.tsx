...
import {
  MAX_FILE_SIZE,
  MAX_FILES,
  createCommercialVehicleSchema,
} from "@/lib/vehicles/commercial-vehicles/types/commercialVehicle.schema";
...
import { publishCommercialVehicleAd } from "@/lib/vehicles/commercial-vehicles/actions/publishCommercialVehicleAd";
import { editCommercialVehicleAd } from "@/lib/vehicles/commercial-vehicles/actions/editCommercialVehicleAd";
...
import {
  ...
  HeroCard,
  SectionCard,
} from "./CommercialVehiclePublishForm.styles";
import {
  mapCommercialVehicleManufacturersToSelectOptions,
} from "@/lib/vehicles/commercial-vehicles/vehicleCommercialManufacturers";
import {
  getCommercialVehicleModelsToSelectOptions,
} from "@/lib/vehicles/commercial-vehicles/vehicleCommercialModels";
...
interface CommercialVehiclePublishFormProps {
  commercialVehicle?: SerializedCommercialVehicle;
  formMode: FormMode;
}

const CommercialVehiclePublishForm: FC<CommercialVehiclePublishFormProps> = ({
  commercialVehicle,
  formMode,
}) => {
  ...
  const updateCommercialVehicleWithImagesToDelete = editCommercialVehicleAd.bind(null, {
    commercialVehiclePublicId: commercialVehicle?.publicId as string,
    imagesToDelete,
    allImagesShouldBeDeleted,
  });

  const [formState, formAction, isPending] = useActionState(
    isCreateMode ? publishCommercialVehicleAd : updateCommercialVehicleWithImagesToDelete,
    undefined
  );

  const [form, fields] = useForm({
    defaultValue: {
      manufacturer: commercialVehicle?.manufacturer || "",
      model: commercialVehicle?.model || "",
      yearOfManufacture: commercialVehicle?.yearOfManufacture?.toString() || "",
      numberOfHand: commercialVehicle?.numberOfHand?.toString() || "",
      transmission: commercialVehicle?.transmission || "",
      mileage: commercialVehicle?.mileage?.toString() || "",
      numberOfDoors: commercialVehicle?.numberOfDoors?.toString() || "",
      color: commercialVehicle?.color || "",
      price: commercialVehicle?.price?.toString() || "",
      description: commercialVehicle?.description || "",
      district: commercialVehicle?.district || Districts.Center,
      city: commercialVehicle?.city || "",
      contactName: commercialVehicle?.contactName || "",
      contactPrimaryPhone: commercialVehicle?.contactPrimaryPhone || "",
      contactSecondaryPhone: commercialVehicle?.contactSecondaryPhone || "",
      contactEmail: commercialVehicle?.contactEmail || user?.email || "",
      acceptTerms: commercialVehicle?.acceptTerms ? "on" : null,
      images: [],
    },
  ...
  const modelOptions = useMemo(
    () =>
      manufacturer.value
        ? getCommercialVehicleModelsToSelectOptions(manufacturer.value as string)
        : [],
    [manufacturer.value]
  );
              ...
              >
                <Box>
                  <Heading as="h1" size={{ initial: "6", md: "7" }}>
                    Расскажите о вашем коммерческом транспорте
                  </Heading>
                  <Text
                    mt="2"
                    size={{ initial: "3", md: "4" }}
            ...
            <SectionCard variant="surface" size="4">
              <Flex direction="column" gap={{ initial: "4", md: "5" }}>
                <Box>
                  ...
                  <Text color="gray" size="2" mt="2">
                    Укажите цену и подробное описание коммерческого транспорта.
                  </Text>
                </Box>
                <Grid
                  columns={{ initial: "1", md: "2" }}
                ...
                <TextAreaField
                  field={description}
                  label="Описание:"
                  placeholder="Опишите коммерческий транспорт..."
                  size="3"
            ...
            <SectionCard variant="surface" size="4">
              <Flex direction="column" gap={{ initial: "4", md: "5" }}>
                <Box>
                  ...
                  <Text color="gray" size="2" mt="2">
                    Укажите район и город, где находится коммерческий транспорт.
                  </Text>
                </Box>
                <Grid
                  columns={{ initial: "1", md: "2" }}
            ...
            <SectionCard variant="surface" size="4">
              <Flex direction="column" gap={{ initial: "4", md: "5" }}>
                <Box>
                  ...
                  <Text color="gray" size="2" mt="2">
                    Добавьте изображения коммерческого транспорта.
...
export default CommercialVehiclePublishForm;
