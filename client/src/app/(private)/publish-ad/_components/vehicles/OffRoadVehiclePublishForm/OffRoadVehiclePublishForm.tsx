...
import {
  MAX_FILE_SIZE,
  MAX_FILES,
  createOffRoadVehicleSchema,
} from "@/lib/vehicles/off-road/types/offRoadVehicle.schema";
...
import { publishOffRoadVehicleAd } from "@/lib/vehicles/off-road/actions/publishOffRoadVehicleAd";
import { editOffRoadVehicleAd } from "@/lib/vehicles/off-road/actions/editOffRoadVehicleAd";
...
import {
  ...
  HeroCard,
  SectionCard,
} from "./OffRoadVehiclePublishForm.styles";
import {
  mapOffRoadVehicleManufacturersToSelectOptions,
} from "@/lib/vehicles/off-road/offRoadVehicleManufacturers";
import {
  getOffRoadVehicleModelsToSelectOptions,
} from "@/lib/vehicles/off-road/offRoadVehicleModels";
...
interface OffRoadVehiclePublishFormProps {
  offRoadVehicle?: SerializedOffRoadVehicle;
  formMode: FormMode;
}

const OffRoadVehiclePublishForm: FC<OffRoadVehiclePublishFormProps> = ({
  offRoadVehicle,
  formMode,
}) => {
  ...
  const updateOffRoadVehicleWithImagesToDelete = editOffRoadVehicleAd.bind(null, {
    offRoadVehiclePublicId: offRoadVehicle?.publicId as string,
    imagesToDelete,
    allImagesShouldBeDeleted,
  });

  const [formState, formAction, isPending] = useActionState(
    isCreateMode ? publishOffRoadVehicleAd : updateOffRoadVehicleWithImagesToDelete,
    undefined
  );

  const [form, fields] = useForm({
    defaultValue: {
      manufacturer: offRoadVehicle?.manufacturer || "",
      model: offRoadVehicle?.model || "",
      yearOfManufacture: offRoadVehicle?.yearOfManufacture?.toString() || "",
      numberOfHand: offRoadVehicle?.numberOfHand?.toString() || "",
      transmission: offRoadVehicle?.transmission || "",
      mileage: offRoadVehicle?.mileage?.toString() || "",
      numberOfDoors: offRoadVehicle?.numberOfDoors?.toString() || "",
      color: offRoadVehicle?.color || "",
      price: offRoadVehicle?.price?.toString() || "",
      description: offRoadVehicle?.description || "",
      district: offRoadVehicle?.district || Districts.Center,
      city: offRoadVehicle?.city || "",
      contactName: offRoadVehicle?.contactName || "",
      contactPrimaryPhone: offRoadVehicle?.contactPrimaryPhone || "",
      contactSecondaryPhone: offRoadVehicle?.contactSecondaryPhone || "",
      contactEmail: offRoadVehicle?.contactEmail || user?.email || "",
      acceptTerms: offRoadVehicle?.acceptTerms ? "on" : null,
      images: [],
    },
  ...
  const modelOptions = useMemo(
    () =>
      manufacturer.value
        ? getOffRoadVehicleModelsToSelectOptions(manufacturer.value as string)
        : [],
    [manufacturer.value]
  );
              ...
              >
                <Box>
                  <Heading as="h1" size={{ initial: "6", md: "7" }}>
                    Расскажите о вашем внедорожнике
                  </Heading>
                  <Text
                    mt="2"
                    size={{ initial: "3", md: "4" }}
            ...
            <SectionCard variant="surface" size="4">
              <Flex direction="column" gap={{ initial: "4", md: "5" }}>
                <Box>
                  ...
                  <Text color="gray" size="2" mt="2">
                    Укажите цену и подробное описание внедорожника.
                  </Text>
                </Box>
                <Grid
                  columns={{ initial: "1", md: "2" }}
                ...
                <TextAreaField
                  field={description}
                  label="Описание:"
                  placeholder="Опишите внедорожник..."
                  size="3"
            ...
            <SectionCard variant="surface" size="4">
              <Flex direction="column" gap={{ initial: "4", md: "5" }}>
                <Box>
                  ...
                  <Text color="gray" size="2" mt="2">
                    Укажите район и город, где находится внедорожник.
                  </Text>
                </Box>
                <Grid
                  columns={{ initial: "1", md: "2" }}
            ...
            <SectionCard variant="surface" size="4">
              <Flex direction="column" gap={{ initial: "4", md: "5" }}>
                <Box>
                  ...
                  <Text color="gray" size="2" mt="2">
                    Добавьте изображения внедорожника.
...
export default OffRoadVehiclePublishForm;
