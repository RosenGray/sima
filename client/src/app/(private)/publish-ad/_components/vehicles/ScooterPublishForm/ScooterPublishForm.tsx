...
import {
  ...
  MAX_FILES,
  createScooterSchema,
} from "@/lib/vehicles/scooters/types/scooter.schema";
...
import { publishScooterAd } from "@/lib/vehicles/scooters/actions/publishScooterAd";
import { editScooterAd } from "@/lib/vehicles/scooters/actions/editScooterAd";
...
import {
  ...
  HeroCard,
  SectionCard,
} from "./ScooterPublishForm.styles";
import {
  mapScooterManufacturersToSelectOptions,
} from "@/lib/vehicles/scooters/scooterManufacturers";
import {
  getScooterModelsToSelectOptions,
} from "@/lib/vehicles/scooters/scooterModels";
import {
  getYearsOptions,
...
interface ScooterPublishFormProps {
  scooter?: SerializedScooter;
  formMode: FormMode;
}

const ScooterPublishForm: FC<ScooterPublishFormProps> = ({ scooter, formMode }) => {
  ...
  const updateScooterWithImagesToDelete = editScooterAd.bind(null, {
    scooterPublicId: scooter?.publicId as string,
    imagesToDelete,
    allImagesShouldBeDeleted,
  });

  const [formState, formAction, isPending] = useActionState(
    isCreateMode ? publishScooterAd : updateScooterWithImagesToDelete,
    undefined
  );
  ...
  const [form, fields] = useForm({
    defaultValue: {
      ...
      model: scooter?.model || "",
      yearOfManufacture: scooter?.yearOfManufacture?.toString() || "",
      numberOfHand: scooter?.numberOfHand?.toString() || "",
      engineCapacity: scooter?.engineCapacity?.toString() || "",
      mileage: scooter?.mileage?.toString() || "",
      price: scooter?.price?.toString() || "",
      description: scooter?.description || "",
      district: scooter?.district || Districts.Center,
      city: scooter?.city || "",
      contactName: scooter?.contactName || "",
      contactPrimaryPhone: scooter?.contactPrimaryPhone || "",
      contactSecondaryPhone: scooter?.contactSecondaryPhone || "",
      contactEmail: scooter?.contactEmail || user?.email || "",
      acceptTerms: scooter?.acceptTerms ? "on" : null,
      images: [],
    },
  ...
  const modelOptions = useMemo(
    () =>
      manufacturer.value
        ? getScooterModelsToSelectOptions(manufacturer.value as string)
        : [],
    [manufacturer.value]
  );
            ...
            <SectionCard variant="surface" size="4">
              <Flex direction="column" gap={{ initial: "4", md: "5" }}>
                <Box>
                  ...
                  <Text color="gray" size="2" mt="2">
                    Укажите цену и подробное описание скутера.
                  </Text>
                </Box>
                <Grid
                  columns={{ initial: "1", md: "2" }}
            ...
            <SectionCard variant="surface" size="4">
              <Flex direction="column" gap={{ initial: "4", md: "5" }}>
                <Box>
                  ...
                  <Text color="gray" size="2" mt="2">
                    Укажите район и город, где находится скутер.
                  </Text>
                </Box>
                <Grid
                  columns={{ initial: "1", md: "2" }}
...
export default ScooterPublishForm;
