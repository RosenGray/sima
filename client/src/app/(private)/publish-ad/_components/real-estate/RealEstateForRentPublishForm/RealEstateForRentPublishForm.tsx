...
} from "@/lib/real-estate/for-rent/types/realEstateForRent.schema";
...
import { publishRealEstateForRentAd } from "@/lib/real-estate/for-rent/actions/publishRealEstateForRentAd";
import { editRealEstateForRentAd } from "@/lib/real-estate/for-rent/actions/editRealEstateForRentAd";
...
import {
  ...
  HeroCard,
  SectionCard,
} from "./RealEstateForRentPublishForm.styles";
...
import {
  ...
  getFloorOptions,
  getBalconyOptions,
  getAnnualPaymentsOptions,
} from "@/lib/real-estate/for-rent/utils/realEstateOptions";

const areasOptions = mapAreasToSelectOptions();

interface RealEstateForRentPublishFormProps {
  realEstate?: SerializedRealEstateForRent;
  formMode: FormMode;
}

const RealEstateForRentPublishForm: FC<RealEstateForRentPublishFormProps> = ({
  realEstate,
  formMode,
}) => {
  ...
  const updateRealEstateWithImagesToDelete = editRealEstateForRentAd.bind(
    null,
    {
  ...
  const [form, fields] = useForm({
    defaultValue: {
      ...
      annualPayments: realEstate?.annualPayments?.toString() || "",
  ...
  const annualPaymentsOptions = useMemo(() => getAnnualPaymentsOptions(), []);
              ...
              >
                <Box>
                  <Heading as="h1" size={{ initial: "6", md: "7" }}>
                    Добавьте объявление о недвижимости
                  </Heading>
                  <Text
                    mt="2"
                    size={{ initial: "3", md: "4" }}
            ...
            <SectionCard variant="surface" size="4">
              <Flex direction="column" gap={{ initial: "4", md: "5" }}>
                <Box>
                  ...
                  <Text color="gray" size="2" mt="2">
                    Укажите цену, налог на недвижимость и количество платежей в год.
                  </Text>
                </Box>
                <Grid
                  columns={{ initial: "1", md: "2" }}
                ...
                >
                  ...
                  <SelectSingle
                    label="Количество платежей в год"
                    field={annualPayments}
                    placeholder="Выберите количество платежей"
                    options={annualPaymentsOptions}
                    errors={annualPayments.errors}
                    isDisabled={isPending}
                    isMandatory
                  />
                </Grid>
...
export default RealEstateForRentPublishForm;
