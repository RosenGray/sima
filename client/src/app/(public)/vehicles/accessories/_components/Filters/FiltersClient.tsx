...
import {
  getAccessoryKindsToSelectOptionsByCategoryIds,
...
import { AccessoryFilter, AccessoryFilterSchema } from "./filters.types";
...
const ENTITY_TYPE = ENTITY_TYPE_VEHICLES_ACCESSORIES;
...
const FiltersClient: FC = () => {
  ...
  const selectedCategoryIds = allSelectedFilterOptions
    .get("category")!
    .map((option) => option.value) as AccessoryCategoryId[];
  ...
  const handleSubmitAllFilters = useCallback(() => {
    ...
    const parseResult = parseWithZod(formData, {
      schema: AccessoryFilterSchema,
    });
    const schemaKeys = Object.keys(AccessoryFilterSchema.shape);
    ...
    if (parseResult.status === "success") {
      ...
      schemaKeys.forEach((key) => {
        ...
        const value = values[key as keyof AccessoryFilter];
  ...
  const kindOptions = useMemo(
    () =>
      getAccessoryKindsToSelectOptionsByCategoryIds(selectedCategoryIds),
    [selectedCategoryIds]
  );
  ...
  useEffect(() => {
    ...
    const allCategoryOptions = mapAccessoryCategoriesToSelectOptions();
    const allKindOptions = getAccessoryKindsToSelectOptionsByCategoryIds(
      searchParams.getAll("category") as AccessoryCategoryId[]
    );
