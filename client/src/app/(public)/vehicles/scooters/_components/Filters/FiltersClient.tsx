...
import {
  getScooterModelsToSelectOptionsByManufacturerIds,
} from "@/lib/vehicles/scooters/scooterModels";
import { ScooterManufacturerId } from "@/lib/vehicles/scooters/scooterManufacturers/types/scooterManufacturer.schema";
import {
  getYearsOptions,
...
import { ScooterFilter, ScooterFilterSchema } from "./filters.types";
...
const ENTITY_TYPE = ENTITY_TYPE_SCOOTERS;
...
const FiltersClient: FC = () => {
  ...
  const selectedManufacturerIds = allSelectedFilterOptions
    .get("manufacturer")!
    .map((option) => option.value) as ScooterManufacturerId[];
  ...
  const handleSubmitAllFilters = useCallback(() => {
    ...
    const parseResult = parseWithZod(formData, { schema: ScooterFilterSchema });
    const schemaKeys = Object.keys(ScooterFilterSchema.shape);
    ...
    if (parseResult.status === "success") {
      ...
      schemaKeys.forEach((key) => {
        ...
        const value = values[key as keyof ScooterFilter];
  ...
  const modelOptions = useMemo(
    () =>
      getScooterModelsToSelectOptionsByManufacturerIds(selectedManufacturerIds),
