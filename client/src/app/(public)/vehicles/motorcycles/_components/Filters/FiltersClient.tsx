...
import { getMotorcycleModelsToSelectOptionsByManufacturerIds } from "@/lib/vehicles/motorcycles/motorcycleModels";
import { VehicleManufacturerId } from "@/lib/vehicles/motorcycles/motorcycleManufacturers/types/motorcycleManufacturer.schema";
import {
  getYearsOptions,
...
const FiltersClient: FC = () => {
  ...
  const handleSubmitAllFilters = useCallback(() => {
    ...
    const parseResult = parseWithZod(formData, {
      schema: MotorcycleFilterSchema,
    });
    const schemaKeys = Object.keys(MotorcycleFilterSchema.shape);
    ...
    if (parseResult.status === "success") {
      ...
      schemaKeys.forEach((key) => {
        ...
        const value = values[key as keyof MotorcycleFilter];
  ...
  const modelOptions = useMemo(
    () =>
      getMotorcycleModelsToSelectOptionsByManufacturerIds(selectedManufacturerIds),
  ...
  const renderMoreFilters = () => {
    return (
      <ModalFiltersSection>
        ...
        <SearchMultiSelect
          placeholder="Тип мотоцикла"
          displayName="тип мотоцикла"
          paramName="kind"
          options={kindOptions}
          maxSelectedOptions={3}
