...
import {
  ...
  HeaderSection,
  ButtonGroup,
} from "./OffRoadVehicleDetailClient.styles";
...
const OffRoadVehicleDetailClient: React.FC<OffRoadVehicleDetailClientProps> = ({ offRoadVehicle }) => {
  ...
  const [formState, formAction, isPending] = useActionState(
    deleteOffRoadVehicleAdByPublicId,
    undefined
  );
  ...
  const isOwner = thisUserIsOwner(offRoadVehicle.user.id);
  ...
  return (
    <PageContainer size="4">
        ...
        {isOwner && (
          <ButtonGroup>
            <Button disabled={isPending} asChild size="3" variant="soft">
              <Link
                color="yellow"
                href={`/publish-ad/vehicles/off-road/edit/${publicId}`}
              >
      ...
      >
        ...
        {/* Information Section */}
        <InfoCard>
          {/* Basic Off-Road Vehicle Information */}
          <InfoSection>
            <InfoTitle size="5">
              <IdCardIcon width="20" height="20" />
              Информация о внедорожнике
            </InfoTitle>
            <InfoRow>
...
export default OffRoadVehicleDetailClient;
