...
import { mapOffRoadVehicleManufacturersToSelectOptions } from "@/lib/vehicles/off-road/offRoadVehicleManufacturers";
import { getOffRoadVehicleModelsToSelectOptionsByManufacturerIds } from "@/lib/vehicles/off-road/offRoadVehicleModels";
import { VehicleManufacturerId } from "@/lib/vehicles/off-road/offRoadVehicleManufacturers/types/offRoadVehicleManufacturer.schema";
import {
  getYearsOptions,
...
import { OffRoadVehicleFilter, OffRoadVehicleFilterSchema } from "./filters.types";
...
const ENTITY_TYPE = ENTITY_TYPE_OFF_ROAD;
...
const FiltersClient: FC = () => {
  ...
  const handleSubmitAllFilters = useCallback(() => {
    ...
    const parseResult = parseWithZod(formData, { schema: OffRoadVehicleFilterSchema });
    const schemaKeys = Object.keys(OffRoadVehicleFilterSchema.shape);
    ...
    if (parseResult.status === "success") {
      ...
      schemaKeys.forEach((key) => {
        ...
        const value = values[key as keyof OffRoadVehicleFilter];
  ...
  const modelOptions = useMemo(
    () =>
      getOffRoadVehicleModelsToSelectOptionsByManufacturerIds(selectedManufacturerIds),
