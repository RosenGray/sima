...
import {
  ...
  HeaderSection,
  ButtonGroup,
} from "./CommercialRealEstateDetailClient.styles";
...
const ADDITIONAL_FEATURES_LABELS: Record<number, string> = {
  ...
  [AdditionalFeatures.Guard]: "Охрана",
  [AdditionalFeatures.Accessible]: "Доступность",
  [AdditionalFeatures.Alarm]: "Сигнализация",
  [AdditionalFeatures.Kitchen]: "Кухня",
  [AdditionalFeatures.MeetingRoom]: "Переговорная",
  [AdditionalFeatures.ReceptionDesk]: "Ресепшн",
};
...
> = ({ commercialRealEstate }) => {
  const deleteCommercialRealEstateAdByPublicId =
    deleteCommercialRealEstateAdWithRedirect.bind(
      null,
      commercialRealEstate.publicId
    );

  const [formState, formAction, isPending] = useActionState(
    deleteCommercialRealEstateAdByPublicId,
    undefined
  );
  ...
  const isOwner = thisUserIsOwner(commercialRealEstate.user.id);
        ...
        {isOwner && (
          <ButtonGroup>
            <Button disabled={isPending} asChild size="3" variant="soft">
              <Link
                color="yellow"
                href={`/publish-ad/real-estate/commercial-real-estate/edit/${publicId}`}
              >
      ...
      >
        ...
        <InfoCard>
          ...
          <InfoSection>
            ...
            <InfoRow>
              <RulerHorizontalIcon width="18" height="18" />
            ...
            <InfoSection>
              ...
              <Flex wrap="wrap" gap="2">
                {additionalFeatures.map((feature) => (
                  <Badge key={feature} size="2" color="blue" variant="soft">
                    {ADDITIONAL_FEATURES_LABELS[feature] || `Особенность ${feature}`}
                  </Badge>
                ))}
              </Flex>
            </InfoSection>
...
export default CommercialRealEstateDetailClient;
