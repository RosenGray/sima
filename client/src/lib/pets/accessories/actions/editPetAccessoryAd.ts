...
export async function editPetAccessoryAd(
  context: {
  ...
  try {
    ...
    const updatedImages = [
      ...accessory.images.filter((img) => !imageIdsToDelete.includes(img.id)),
      ...uploadResult.files,
    ];

    // Update accessory using repository
    const formDataValue = result.value as PetAccessoryFormData;
    const { images: _, ...accessoryData } = formDataValue;
    await petAccessoryRepository.edit(context.entityPublicId, {
      ...accessoryData,
