...
export async function deletePetAccessoryAd(accessoryPublicId: string) {
  try {
    ...
    const accessory = await petAccessoryRepository.getByPublicId(
      accessoryPublicId
    );
    ...
    const deleted = await petAccessoryRepository.delete(accessoryPublicId);
    ...
    console.error("Error deleting pet accessory ad:", error);
    return {
      success: false,
      error: "Ошибка при удалении объявления. Попробуйте позже.",
    };
...
export async function deletePetAccessoryAdWithRedirect(
  accessoryPublicId: string
) {
  const result = await deletePetAccessoryAd(accessoryPublicId);

  if (result.success) {
    revalidatePath("/pets/accessories");
