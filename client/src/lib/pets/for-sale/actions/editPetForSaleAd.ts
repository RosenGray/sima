...
export async function editPetForSaleAd(
  context: {
    petPublicId: string;
  ...
  try {
    ...
    // Get existing pet using repository
    const pet = await petForSaleRepository.getByPublicId(context.petPublicId);
    if (!pet) {
      return result.reply({
    ...
    // Update pet using repository
    const formDataValue = result.value as PetForSaleFormData;
    const { images: _, ...petData } = formDataValue;
    await petForSaleRepository.edit(context.petPublicId, {
      ...petData,
