...
// Generate annualPayments options: 1-12
export const ANNUAL_PAYMENTS_OPTIONS = Array.from({ length: 12 }, (_, i) => i + 1);

export const createRealEstateForRentSchema = ({
  minNumberOfImages = 1,
}) => {
  return z.object({
    ...
    annualPayments: z.coerce
      .number({
        required_error: "Выберите количество платежей в год",
        invalid_type_error: "Количество платежей должно быть числом",
      })
      .refine((val) => ANNUAL_PAYMENTS_OPTIONS.includes(val), {
        message: "Неверное значение количества платежей",
      }),
    year: z.coerce
      .number({
...
export type RealEstateForRentFormData = z.infer<
  ReturnType<typeof createRealEstateForRentSchema>
>;
