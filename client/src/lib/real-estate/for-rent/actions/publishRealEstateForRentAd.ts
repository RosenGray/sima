...
export async function publishRealEstateForRentAd(
  initialState: unknown,
  formData: FormData
) {
  const result = parseWithZod(formData, {
    schema: createRealEstateForRentSchema({ minNumberOfImages: 1 }),
  });
  ...
  const formDataValue = result.value as unknown as RealEstateForRentFormData;
  ...
  try {
    ...
    await realEstateForRentRepository.create({
      ...realEstateData,
