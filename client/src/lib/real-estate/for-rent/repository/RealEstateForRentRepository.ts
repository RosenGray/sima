...
export interface RealEstateForRentSearchFilters {
  propertyKind?: string[];
  district?: string[];
  city?: string[];
...
interface PaginatedResponse {
  data: SerializedRealEstateForRent[];
  totalCount: number;
  currentPage: number;
  totalPages: number;
  hasNextPage: boolean;
...
class RealEstateForRentRepository {
  /**
   * Get all real estate for rent with search and pagination
   ...
   */
  async getAll(
    searchFilters: RealEstateForRentSearchFilters = {},
    ...
    try {
      ...
      const sanitizedFilters: RealEstateForRentSearchFilters = {
      ...
      const totalCount = await RealEstateForRent.countDocuments(searchFilter);
      const totalPages = Math.ceil(totalCount / pageSize);

      // Fetch paginated results
      const realEstates = await RealEstateForRent.find(searchFilter)
        .populate("user")
      ...
      console.error("Error fetching real estate for rent:", error);
      throw new Error("Failed to fetch real estate for rent");
    }
  }
   ...
   * @returns Promise<SerializedRealEstateForRent | null> - The real estate or null if not found
   */
  async getById(id: string): Promise<SerializedRealEstateForRent | null> {
    try {
      await connectDB();
      ...
      const realEstate = await RealEstateForRent.findById(id).populate("user");
  ...
  /**
   * Create a new real estate for rent
   * @param realEstateData - Real estate data to create
   * @returns Promise<SerializedRealEstateForRent> - The created real estate
   */
  async create(
    realEstateData: Omit<
      IRealEstateForRent,
    ...
    try {
      ...
      const realEstate = new RealEstateForRent(realEstateData);
      ...
      console.error("Error creating real estate for rent:", error);
      throw new Error("Failed to create real estate for rent");
    }
  }

  /**
   * Update a real estate for rent by publicId
   * @param publicId - The public ID of the real estate to update
   * @param updateData - Partial real estate data to update
   * @returns Promise<SerializedRealEstateForRent | null> - The updated real estate or null if not found
   */
  async edit(
    publicId: string,
    updateData: Partial<IRealEstateForRent>
  ): Promise<SerializedRealEstateForRent | null> {
    try {
      await connectDB();

      const realEstate = await RealEstateForRent.findOneAndUpdate(
        { publicId },
        { ...updateData },
      ...
      console.error("Error updating real estate for rent:", error);
      throw new Error("Failed to update real estate for rent");
    }
  }

  /**
   * Delete a real estate for rent by publicId
  ...
  async delete(publicId: string): Promise<boolean> {
    try {
      ...
      const result = await RealEstateForRent.findOneAndDelete({
        publicId,
      });

      return result !== null;
    } catch (error) {
      console.error("Error deleting real estate for rent:", error);
      throw new Error("Failed to delete real estate for rent");
    }
  }
}

export const realEstateForRentRepository = new RealEstateForRentRepository();
