...
export async function publishScooterAd(initialState: unknown, formData: FormData) {
  const result = parseWithZod(formData, {
    schema: createScooterSchema({ minNumberOfImages: 1 }),
  });
  ...
  const formDataValue = result.value as unknown as ScooterFormData;
  const { images, ...scooterData } = formDataValue;
  ...
  try {
    ...
    // Create scooter using repository
    await scooterRepository.create({
      ...scooterData,
