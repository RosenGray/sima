...
export const createScooterSchema = ({ minNumberOfImages = 1 }) => {
  return z.object({
    ...
    engineCapacity: z.coerce
      .number({
        required_error: "Введите объем двигателя",
        invalid_type_error: "Объем двигателя должен быть числом",
      })
      .min(1, "Объем двигателя должен быть не менее 1"),
    mileage: z.coerce
      .number({
...
export type ScooterFormData = z.infer<ReturnType<typeof createScooterSchema>>;
